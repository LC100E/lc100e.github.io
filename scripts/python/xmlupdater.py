...
def generate_seo_attributes(item_title, pdf_url, parent_titles):
    """Generates cleanurlslug, fulltitle, and description for an item."""

    # 1. Generate cleanurlslug
    if pdf_url.startswith('/pdf/'):
        pdf_path_part = pdf_url.replace('/pdf/', '').replace('.pdf', '.')
        pdf_slug_base = sanitize_for_slug(pdf_path_part.replace('/', '-'))
    else: # For special URLs like /images/index.pdf (HOME)
        pdf_slug_base = sanitize_for_slug(pdf_url.replace('/', '-').replace('.pdf', '.'))

    serving_html_slug = get_serving_html(pdf_url)
    ...
    clean_url_slug = f"{serving_html_slug}?page={_global_node_id_counter}-{parent_title_slug}-{title_slug}" if title_slug else ""
    if not clean_url_slug:
        clean_url_slug = "untitled-page"

    if item_title == "HOME" and pdf_url == "/images/index.pdf":
...
def build_nested_json(element, parent_titles=None):
    ...
    if element.tag == 'item':
        ...
        if 'title' in node and 'url' in node:
            seo_attrs = generate_seo_attributes(node['title'], node['url'], parent_titles)
...
output_json_path = 'xml/menu_data.json'
