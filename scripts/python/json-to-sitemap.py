...
from urllib.parse import urljoin # For safely joining base URL with relative path

# --- Configuration ---
# Base URL for your GitHub Pages site.
# This should be the root of your site (e.g., "https://lc100e.github.io/"),
# as path is expected to be a relative path (e.g., "/some/path/doc.html").
BASE_URL = "https://lc100e.github.io/"

# Path to your generated menu_data.json file
MENU_DATA_PATH = 'src/xml/index.json'
# Output sitemap file name
SITEMAP_OUTPUT_PATH = 'src/public/sitemap.xml'
...
def collect_urls_from_menu_data(node, urls_list):
    ...
    It now uses the 'path' property directly to form the sitemap entry URL.
    """
    if node.get('type') == 'item':
        path = node.get('path')
        
        if path:
            # Use urljoin to safely combine the BASE_URL with the relative path.
            # This handles cases where path might start with '/' or not,
            # and ensures correct slashes between BASE_URL and the path.
            full_item_url = urljoin(BASE_URL, path)
...
if __name__ == "__main__":
    try:
        ...
        # Add all collected item URLs from path
